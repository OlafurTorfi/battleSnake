...
function decideWhichMove(forwardState, leftState, rightState, forwardRandom, leftRandom, rightRandom) {
    ...
    if (forwardState &&
        ...
        console.log("forward because fVal " + forwardState.mCount.current * forwardRandom + ", rVal " + (rightState &&
            rightState.mCount.current * rightRandom) + ", lVal " + (leftState &&
            leftState.mCount.current * leftRandom) + ". And Move is therefore " + moveOnMap_1.moveOnMap(forwardState.lastMove) + " based on " + JSON.stringify(forwardState.lastMove));
    ...
    if (leftState && (!rightState || leftState.mCount.current * leftRandom > rightState.mCount.current * rightRandom)) {
        console.log("left because fVal " + (forwardState && forwardState.mCount.current * forwardRandom) + ", rVal " + (rightState &&
            rightState.mCount.current * rightRandom) + ", lVal " + (leftState &&
            leftState.mCount.current * leftRandom) + ". And Move is therefore " + moveOnMap_1.moveOnMap(leftState.lastMove) + " based on " + JSON.stringify(leftState.lastMove));
        return moveOnMap_1.moveOnMap(leftState.lastMove);
    }
    if (rightState) {
        console.log("right because fVal " + (forwardState && forwardState.mCount.current * forwardRandom) + ", rVal " + (rightState &&
            rightState.mCount.current * rightRandom) + ", lVal " + (leftState &&
            leftState.mCount.current * leftRandom) + ". And Move is therefore " + moveOnMap_1.moveOnMap(rightState.lastMove) + " based on " + JSON.stringify(rightState.lastMove));
